...
bool TextureShader::InitializeShader(ID3D11Device * device, HWND hwnd, WCHAR * vsFilename, WCHAR * psFilename)
{
	...
	//Initialize the pointers this funtions will use to null.
	errorMessage = 0;
	vertexShaderBuffer = 0;
	pixelShaderBuffer = 0;

	//Compile the vertex shader code
	D3DX11CompileFromFile(vsFilename, NULL, NULL, "TextureVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
				       &vertexShaderBuffer, &errorMessage, &result);
	if (FAILED(result))
	{
	...
	D3DX11CompileFromFile(psFilename, NULL, NULL, "TexturePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
						&pixelShaderBuffer, &errorMessage, &result);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
	...
	//Setup the description of the dynamic matrix constant buffer interface and then use that is in the vertex shader.
...
void TextureShader::OutputShaderErrorMessage(ID3D10Blob * errorMessage, HWND hwnd, WCHAR *shaderFilename)
{
	...
	MessageBox(hwnd, L"Error compiling shade. Check shader-error.txt for message.", shaderFilename, MB_OK);
